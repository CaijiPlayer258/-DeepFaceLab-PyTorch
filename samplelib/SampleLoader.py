    ...
    def load(sample_type, samples_path, subdirs=False):
        ...
        if            sample_type == SampleType.IMAGE:
            if  samples[sample_type] is None:
                samples[sample_type] = [ Sample(filename=filename) for filename in io.progress_bar_generator( pathex.get_image_paths(Path(samples_path), subdirs=subdirs), "Loading") ]

        elif          sample_type == SampleType.FACE:
            if  samples[sample_type] is None:
                try:
                    result = samplelib.PackedFaceset.load(Path(samples_path))
                ...
                if result is None:
                    result = SampleLoader.load_face_samples( pathex.get_image_paths(Path(samples_path), subdirs=subdirs) )
