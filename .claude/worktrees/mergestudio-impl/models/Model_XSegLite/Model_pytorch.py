...
from samplelib import DynamicSampleGenerator, SampleGeneratorFace, SampleProcessor
...
class XSegLiteModel(ModelBase):
    ...
    def on_initialize_options(self):
        ...
        if self.is_first_run() or ask_override:
            ...
            self.options['loader_skip'] = io.input_bool(
                "Skip original loader (fast, no validation)", False,
    ...
    def on_initialize(self):
        ...
        io.log_info('')
        io.log_info('============================================================')
        io.log_info('  torch.compile AUTOTUNE 即将开始')
        io.log_info('  此过程需要 GPU 完全空闲以获得最佳 kernel 选型')
        io.log_info('  请关闭：游戏、浏览器视频、直播、远程桌面 等 GPU 负载')
        io.log_info('  AUTOTUNE 期间 GPU 会跑满，这是正常的')
        io.log_info('  编译完成后即可正常使用电脑')
        io.log_info('============================================================')
        io.log_info('')
        if not io.input_bool("已关闭非必要程序，继续?", True):
            raise SystemExit(0)
        try:
            self._compiled = torch.compile(self._model, mode='max-autotune', dynamic=True)
            io.log_info('torch.compile AUTOTUNE 完成')
        except Exception:
            pass
        ...
        if self.is_training:
            ...
            if self.pretrain:
                ...
                srcdst_generator = DynamicSampleGenerator(
                ...
                self.set_training_data_generators([srcdst_generator])
