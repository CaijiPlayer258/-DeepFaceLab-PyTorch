...
class ModelBase(object):
    ...
    def ask_crash_threshold(self, default_value=2.0):
        default_crash = self.load_or_def_option('crash_threshold', default_value)
        self.options['crash_threshold'] = io.input_number(f"崩溃阈值", default_crash, add_info="0=关闭, 建议2.0", help_message="当 src 或 dst loss 超过此值时视为训练异常，自动禁用保存与备份。0 表示关闭。")
