...
class SAEHDModel(ModelBase):
    ...
    def export_dfm(self):
        ...
        # FP16/BF16 on CPU is extremely slow; use GPU if available
    ...
    def onGetPreview(self, samples, for_history=False):
        ...
        # Convert target masks to NHWC before DDM/SDM repeat (1-channel masks)
        tgt_srcm_nhwc = nn.to_data_format(target_srcm, 'NHWC', self.model_data_format)  # (N, H, W, 1)
        tgt_dstm_nhwc = nn.to_data_format(target_dstm, 'NHWC', self.model_data_format)

        # Save 1-channel DDM/SDM for preview mask compositing
        ddm_1ch = DDM.copy()
        sdm_1ch = SDM.copy()
        ...
        for i in range(n_samples):
            if self.face_type < FaceType.HEAD:
                ...
                src_merge_mask = target_srcm[i] * sdm_1ch[i]
            else:
