...
Architecture: Encoder → inter_AB + inter_B → single Decoder (LIAE style).
Same convolutional backbone as DeepFakeLarge but with dual intermediate
bottlenecks and a shared decoder, enabling better identity separation.
"""
...
class Encoder(torch.nn.Module):
    """Encoder with Inception-style first layer, 5 stages Conv3x3x2 + avg_pool."""
    ...
    def __init__(self, resolution, e_dims):
        ...
        c2 = max(1, ed // 3)
        c3 = max(1, ed - c1 - c2)
        self.d1_c1 = ConvAct(3, c1, 1)
        self.d1_mid = ConvAct(3, ed, 1)
        self.d1_c32 = ConvAct(ed, c2, 3)
        self.d1_c51 = ConvAct(3, ed, 1)
        self.d1_c52 = ConvAct(ed, ed, 3)
        self.d1_c53 = ConvAct(ed, c3, 3)

        self.d2_c1 = ConvAct(ed, int(ed * 1.5), 3)
        self.d2_c2 = ConvAct(int(ed * 1.5), ed * 2, 3)
        self.d3_c1 = ConvAct(ed * 2, ed * 3, 3)
        self.d3_c2 = ConvAct(ed * 3, ed * 4, 3)
        self.d4_c1 = ConvAct(ed * 4, ed * 6, 3)
        self.d4_c2 = ConvAct(ed * 6, ed * 8, 3)
        self.d5_c1 = ConvAct(ed * 8, ed * 6, 3)
        self.d5_c2 = ConvAct(ed * 6, ed * 8, 3)

    def forward(self, x):
        shapes = []
        b0 = self.d1_c1(x)
        b1 = self.d1_c32(self.d1_mid(x))
        b2 = self.d1_c53(self.d1_c52(self.d1_c51(x)))
        x = torch.cat([b0, b1, b2], dim=1)
        shapes.append(('d1', x.shape))
        x = F.avg_pool2d(x, 2)

        x = self.d2_c2(self.d2_c1(x))
        shapes.append(('d2', x.shape))
        x = F.avg_pool2d(x, 2)

        x = self.d3_c2(self.d3_c1(x))
        shapes.append(('d3', x.shape))
        x = F.avg_pool2d(x, 2)

        x = self.d4_c2(self.d4_c1(x))
        shapes.append(('d4', x.shape))
        x = F.avg_pool2d(x, 2)

        x = self.d5_c2(self.d5_c1(x))
        shapes.append(('d5', x.shape))
        x = F.avg_pool2d(x, 2)
...
class LIAELarge(torch.nn.Module):
    """LIAE-style network: Encoder → inter_AB + inter_B → single Decoder.

    inter_AB learns shared identity features.
    inter_B learns dst-specific features.
    Decoder reconstructs from concatenated intermediate codes.
    """
    ...
    def __init__(self, resolution=256, ae_dims=256, e_dims=64, d_dims=64):
        ...
        enc_ch = e_dims * 8
        inter_out = ae_dims * 2
        self.inter_AB = InterBottleneck(enc_ch, resolution, ae_dims, out_ch=inter_out)
        self.inter_B = InterBottleneck(enc_ch, resolution, ae_dims, out_ch=inter_out)
        self.decoder = Decoder(d_dims, width=1.0, in_ch=inter_out * 2)

    def forward(self, x):
        enc, s1 = self.encoder(x)
        ab, z_ab = self.inter_AB(enc)
        b, z_b = self.inter_B(enc)
        z = torch.cat([ab, b], dim=1)
        face, mask, s2 = self.decoder(z)
        return {
            'face': face, 'mask': mask,
            'z_ab': z_ab, 'z_b': z_b,
            'inter_AB': ab, 'inter_B': b,
            'shapes': s1 + [('inter_AB', ab.shape), ('inter_B', b.shape),
...
class LIAELargeModel(ModelBase):
    ...
    def on_initialize(self):
        ...
        if self.is_training:
            ...
            self.optimizer = OptimizerClass(
                self.net.parameters(), lr=lr, betas=(0.5, 0.999),
            )
            if gan_power != 0.0:
        ...
        _load_pth('inter_AB.pth', self.net.inter_AB, 'Inter_AB')
        _load_pth('inter_B.pth', self.net.inter_B, 'Inter_B')
    ...
    def get_model_filename_list(self):
        files = ['encoder.pth', 'inter_AB.pth', 'inter_B.pth', 'decoder.pth']
    ...
    def onSave(self):
        ...
        self._save_pth('inter_AB.pth', self.net.inter_AB.state_dict())
        self._save_pth('inter_B.pth', self.net.inter_B.state_dict())
    ...
    def _forward(self, warped_src, warped_dst):
        # LIAE forward: encoder → inter_AB + inter_B → decoder
        src_enc, _ = self.net.encoder(warped_src)
        src_ab, _ = self.net.inter_AB(src_enc)
        src_cat = torch.cat([src_ab, src_ab], dim=1)

        dst_enc, _ = self.net.encoder(warped_dst)
        dst_b, _ = self.net.inter_B(dst_enc)
        dst_ab, _ = self.net.inter_AB(dst_enc)
        ...
        pred_src_dst_no_code_grad, _ = self.net.decoder(src_dst_cat.detach())
    ...
    def onGetPreview(self, samples, for_history=False):
        ((warped_src, target_src, target_srcm, _),
         (warped_dst, target_dst, target_dstm, _)) = samples
    ...
    def AE_merge(self, warped_dst):
        ...
        with torch.no_grad():
            ...
            ab, _ = self.net.inter_AB(enc)
            b, _ = self.net.inter_B(enc)
