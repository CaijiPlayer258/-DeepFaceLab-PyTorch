...
try:
    from WebUI import page_sample_viz as _viz_mod
except ImportError:
    try:
...
body{background:#0f0f13;color:#e0e0e0;font-family:'Segoe UI',system-ui,sans-serif;min-height:100vh;overflow-x:hidden}
header{display:flex;align-items:center;gap:16px;padding:14px 24px;background:#16161e;border-bottom:1px solid #2a2a3a;flex-wrap:wrap}
header h1{font-size:18px;font-weight:600;color:#7eb8f7}
#dot{width:10px;height:10px;border-radius:50%;background:#444;flex-shrink:0;transition:background .4s}
...
#stats{display:flex;gap:20px;margin-left:auto;font-size:13px;color:#aaa;flex-wrap:wrap;align-items:center}
#stats span b{color:#e0e0e0}
button{padding:6px 14px;background:#2a2a3a;border:1px solid #3a3a5a;border-radius:6px;color:#7eb8f7;cursor:pointer;font-size:13px}
button:hover{background:#3a3a5a}
button:disabled{opacity:.4;cursor:default}
#btn-auto-refresh.active{background:#2a5a3a;border-color:#3a7a5a;color:#7ef7a0}
main{display:grid;grid-template-columns:1fr;gap:20px;padding:20px 24px}
.card{background:#16161e;border:1px solid #2a2a3a;border-radius:10px;padding:16px}
.card h2{font-size:13px;font-weight:600;color:#888;text-transform:uppercase;letter-spacing:.8px;margin-bottom:12px}
...
#prev-tabs{display:flex;gap:6px;margin-bottom:12px;flex-wrap:wrap}
.prev-tab{padding:5px 14px;background:#1e1e2e;border:1px solid #3a3a5a;border-radius:6px;color:#888;cursor:pointer;font-size:12px;transition:all .2s}
.prev-tab:hover{background:#2a2a3a;color:#bbb}
.prev-tab.active{background:#2a3a5a;border-color:#4a7ab7;color:#7eb8f7}
...
#preview-img{width:100%;height:auto;border-radius:6px;border:1px solid #2a2a3a;cursor:zoom-in;display:block}
...
.label-grad{background:linear-gradient(90deg,#4a6ab7,#b07eff,#4a6ab7);background-size:200% 100%;-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;animation:gradShift 4s steps(120) infinite;filter:drop-shadow(0 0 4px rgba(0,0,0,.9)) drop-shadow(0 0 8px rgba(0,0,0,.6))}
#prev-hint{font-size:11px;color:#555;margin-top:6px}
#log{font-size:12px;color:#888;font-family:monospace;max-height:100px;overflow-y:auto;line-height:1.6}
...
@media(max-width:600px){header{padding:10px 12px}main{padding:12px}.card{padding:12px}#stats{gap:10px}}
/* --- sidebar --- */
#sidebar{position:fixed;top:0;left:-260px;width:260px;height:100vh;background:#13131f;border-right:1px solid #2a2a3a;z-index:200;transition:left .25s ease;display:flex;flex-direction:column;padding:0;overflow-y:auto}
#sidebar.open{left:0}
#sidebar .shead{padding:16px 18px;border-bottom:1px solid #2a2a3a}
#sidebar .shead h2{font-size:14px;font-weight:600;color:#e0e0e0}
#sidebar .shead .ssub{font-size:11px;color:#666;margin-top:4px}
#sidebar .shead .ssub span{color:#aaa}
#sidebar .snav{flex:1;padding:10px 0}
#sidebar .snav a{display:flex;align-items:center;gap:10px;padding:10px 18px;color:#aaa;text-decoration:none;font-size:13px;transition:all .15s;border-left:3px solid transparent}
#sidebar .snav a:hover{background:#1a1a2e;color:#e0e0e0;border-left-color:#4a7ab7}
#sidebar .snav a.active{color:#7eb8f7;border-left-color:#7eb8f7}
#sidebar-overlay{position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:199;display:none}
#sidebar-overlay.open{display:block}
#hamburger{display:inline-flex;align-items:center;justify-content:center;width:32px;height:32px;background:transparent;border:1px solid #3a3a5a;border-radius:6px;color:#aaa;cursor:pointer;font-size:18px;line-height:1;padding:0;flex-shrink:0;transition:all .15s}
#hamburger:hover{background:#2a2a3a;color:#e0e0e0}
</style>
</head>
<body>
<!-- Password modal -->
...
function renderLabels() {
  ...
  _labelRects = [];
  if (_labelAnimId) { cancelAnimationFrame(_labelAnimId); _labelAnimId = null; }
  if (!_previewLabels || !_previewLabels.n_samples || !_previewLabels.n_samples > 0) return;
  var tabName = (_allPreviews[_currentView] || {}).name || '';
  var isMorph = tabName.indexOf('morph') !== -1;
  var nRows = _previewLabels.n_samples;
  ...
  for (let r = 0; r < nRows; r++) {
    ...
    if (isMorph) {
      ...
      rowLabel = function(col) {
        switch (col) {
          case 0: return sRaw;
          case 1: return fmtLoss(sLossVec ? sLossVec[rowIdx] : sLoss);
          case 2: return dRaw;
          case 3: return fmtLoss(dLossVec ? dLossVec[rowIdx] : dLoss);
          case 4: return 'pred';
        }
        return '';
      };
...
class Handler(BaseHTTPRequestHandler):
    ...
    def log_message(self, fmt, *args):
        try:
            ...
            msg = f'[WebUI]{ip}（{port}）（{loc}）{args[0] if args else fmt}'
            print(msg)
        except Exception:
            pass
