...
class Conv2D(nn.LayerBase):
    ...
    def forward(self, x):
        ...
        if self._same_padding:
            # Compute TF-style SAME padding based on actual input size
            h_in, w_in = x.shape[2], x.shape[3]
            h_out = (h_in + self.strides - 1) // self.strides
            w_out = (w_in + self.strides - 1) // self.strides
            pad_h = max(0, (h_out - 1) * self.strides + self.kernel_size - h_in)
            pad_w = max(0, (w_out - 1) * self.strides + self.kernel_size - w_in)
