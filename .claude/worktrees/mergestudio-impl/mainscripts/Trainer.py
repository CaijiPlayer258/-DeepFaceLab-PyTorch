        ...
        try:
            ...
            def send_preview():
                if not debug:
                    ...
                    if previews:
                        ...
                        # n_samples always from 5-column layout (mask data dimension)
                        cell_h = p_w // 5
            ...
            def _push_settings_to_cache(model, webui):
                ...
                try:
                    ...
                    for k in ('crash_threshold', 'backup_interval', 'max_backups',
                              ...
                              'lr_dropout', 'use_bf16', 'models_opt_on_gpu', 'random_warp', 'random_hsv_power',
                              ...
                              'bg_style_power', 'masked_training', 'write_preview_history', 'target_iter'):
            ...
            def _apply_model_options(model, updates):
                for k, v in updates.items():
                    if k not in ('crash_threshold', 'backup_interval', 'max_backups',
        ...
        while True:
            ...
            if not c2s.empty():
                ...
                if op == 'show':
                    ...
                    cached_src_loss = f"{input['src_loss']:.4f}" if 'src_loss' in input else ''
                    cached_dst_loss = f"{input['dst_loss']:.4f}" if 'dst_loss' in input else ''
                    ...
                    if _webui is not None:
                        ...
                        if _webui_preview_pending:
                            try:
                                encoded = {}
                                for name, bgr in previews:
                                    if cached_n_samples > 0 and (cached_src_loss or cached_dst_loss):
                                        bgr_copy = bgr.copy()
                                        _ncols = cached_n_cols if cached_n_cols else 5
                                        _cw = bgr_copy.shape[1] // _ncols
                                        _ch = bgr_copy.shape[0] // cached_n_samples
                                        for _i in range(cached_n_samples):
                                            _y0 = _i * _ch
                                            if _ncols == 5:
                                                # SAEHD: col2=src_loss, col4=dst_loss
                                                if cached_src_loss:
                                                    _draw_dynamic_text(bgr_copy[_y0:_y0+_ch, _cw:2*_cw], cached_src_loss, time.time() % 1000, 'bottom')
                                                if cached_dst_loss:
                                                    _draw_dynamic_text(bgr_copy[_y0:_y0+_ch, 3*_cw:4*_cw], cached_dst_loss, time.time() % 1000, 'bottom')
                                            else:
                                                # XSegLite: col1=filename, col4=per-sample loss
                                                _fn = (cached_src_fnames[_i] if _i < len(cached_src_fnames) else '')
                                                if _fn:
                                                    _fn = os.path.basename(str(_fn))
                                                    _draw_dynamic_text(bgr_copy[_y0:_y0+_ch, 0*_cw:1*_cw], _fn, time.time() % 1000, 'bottom')
                                                _cl = (cached_src_loss_vec[_i] if _i < len(cached_src_loss_vec) else None)
                                                _sl = f'{_cl:.4f}' if _cl is not None else ''
                                                if _sl:
                                                    _draw_dynamic_text(bgr_copy[_y0:_y0+_ch, 3*_cw:4*_cw], _sl, time.time() % 1000, 'bottom')
                                        img = (np.clip(bgr_copy, 0, 1) * 255).astype(np.uint8)
                                    else:
                                        img = (np.clip(bgr, 0, 1) * 255).astype(np.uint8)
                                    ok, jpg = cv2.imencode('.jpg', img, [cv2.IMWRITE_JPEG_QUALITY, 100])
                                    if ok:
                                        encoded[name] = jpg.tobytes()
                                _webui.update_cache({'previews': encoded})
                            finally:
                                _webui_preview_pending = False
            ...
            if cached_base is not None:
                ...
                if prev_start > 0 and cached_n_samples > 0:
                    ...
                    cell_h = ph // cached_n_samples
                    # Use n_cols from payload; SAEHD defaults to 5
                    n_cols = cached_n_cols if cached_n_cols else 5
                    cell_w = pw // n_cols
                    cell_h = ph // cached_n_samples

                    # "遮罩下"标签：列1/3/4/5 用真实遮罩做羽化合成 + 科技感灰度渐变（30 FPS）
                    if '遮罩' in selected_preview_name and cached_mask_data is not None:
                        _t = time.time()
                        _col_keys = ['col0', 'col2', 'col3', 'col4']
                        _col_x = [0, 2, 3, 4]
                        for i in range(cached_n_samples):
                            y0 = prev_start + i * cell_h
                            bg = _make_chaotic_diffusion((cell_h, cell_w), _t + i * 0.3)
                            for _ck, _cx in zip(_col_keys, _col_x):
                                col = final[y0:y0+cell_h, _cx*cell_w:(_cx+1)*cell_w]
                                mo = cv2.resize(cached_mask_data[_ck][i], (cell_w, cell_h),
                                                interpolation=cv2.INTER_LINEAR)
                                mo = mo[..., None] if mo.ndim == 2 else mo
                                # col0/col2 use binary dataset masks whose crop-edge pixels
                                # are dark; erode the alpha (but not the reconstruction mask)
                                # to push feathering inward where face pixels are brighter
                                if _ck in ('col0', 'col2'):
                                    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (7, 7))
                                    mb = cv2.GaussianBlur(cv2.erode(mo, kernel), (0, 0), sigmaX=2.0)
                                else:
                                    mb = cv2.GaussianBlur(mo, (0, 0), sigmaX=2.0)
                                mb = mb[..., None] if mb.ndim == 2 else mb
                                col[:] = (col / np.clip(mo, 1e-6, None)) * mb + bg * (1.0 - mb)
